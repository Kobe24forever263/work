...
\begin{abstract}
Multi-floor warehouses couple task allocation with transport-mode selection: wheeled robots move efficiently on level ground, legged robots traverse stairs, and cross-floor deliveries may use one legged robot or a wheeled--legged--wheeled relay. We formulate this problem as an event-driven semi-Markov decision process with asynchronous arrivals, variable-duration actions, persistent robot locations, and concurrent resource use. At each decision epoch, a centralized policy selects a task, mode, participating robots, and stairway. A feasibility layer enumerates joint candidates and masks conflicts before sampling. A shared actor combines global context with per-candidate geometric, temporal, resource, and handover-risk features. Proximal policy optimization uses duration-aware discounting and generalized advantage estimation. In matched-load locked evaluations across \textsc{Medium}, \textsc{Dense}, and \textsc{Burst}, 30 independently trained fixed-profile policies achieved 96.30--98.08\% success, and their return and successful-throughput point estimates exceeded those of a time-greedy rule in all three profiles. In a separate randomized persistent mixed-load evaluation, 10 independently trained mixed-curriculum policies maintained 96.51\% success, statistically comparable to the rule, while improving raw return by 89.46 and reducing mean waiting by 30.14~s; relative to a single-dog control, mean waiting decreased by 178.88~s. The results show that the proposed scheduling framework remains effective under both fixed-profile and mixed-load operation, although load-matched specialists can retain an efficiency advantage on some metrics. The policy operates only at the scheduling layer; navigation and robot control remain separate.
\end{abstract}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig_architecture_drawio.pdf}
...
We presented an event-driven formulation for persistent heterogeneous transport in a multi-floor warehouse. The approach couples task selection, transport-mode choice, robot assignment, and stair reservation in one masked candidate action. A shared context--candidate actor avoids index-specific action parameters, while duration-aware discounting aligns PPO updates with elapsed execution time. In matched-load locked logical-simulation tests, the three fixed-profile cohorts achieved 96.30--98.08\% success and exceeded the time-greedy rule in return and successful-throughput point estimates across all profiles. A separately trained mixed-curriculum cohort maintained 96.51\% success under randomized persistent mixed-load streams while reducing mean waiting and improving return relative to time-greedy and single-dog controls. Together, these results show that the proposed masked SMDP-PPO scheduling framework performs effectively under both fixed-profile and mixed-load operation. Load-matched specialists nevertheless remained more efficient on some metrics, so the evidence supports robustness across operating regimes rather than universal dominance by one policy. These results establish multi-seed scheduling evidence, but not navigation, contact-dynamics, or hardware validity. Future work will complete closed-loop navigation, physical handover validation, and robustness tests under partial observability and robot failures.
