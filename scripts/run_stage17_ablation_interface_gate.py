#!/usr/bin/env python3
"""Verify Stage 17 observation, architecture, and discount ablations."""
...
def main() -> int:
    ...
    diagnostics = {}
    expected_state = {
        "NO_QUEUE_RESOURCE": set(
            Stage12Encoder.queue_resource_state_indices()),
        "NO_HANDOVER_CUES": set(),
        "NO_PERSISTENT_POSITION": set(
            Stage12Encoder.persistent_position_state_indices()),
    }
    expected_action = {
        "NO_QUEUE_RESOURCE": set(),
        "NO_HANDOVER_CUES": {28, 29},
        "NO_PERSISTENT_POSITION": set(
            Stage12Encoder.persistent_position_action_indices()),
    }
    ...
    output = (WORK_ROOT / "results" / "stage17" /
              "stage17_ablation_interface_gate.summary.json")
    report = {
        "stage": 17,
        "gate": "ABLATION_INTERFACE_AND_CAUSALITY",
        "claim_boundary": (
